...
from need.models.params import StateDim, ActionDim
from need.env.obsToState import ObsToState
...
class MyCombatEnv(Env):
    def __init__(self, role='red', role_id='1001', env_id=0, host='127.0.0.1', port=8868, render_mode=None):
        ...
        # 使用连续状态空间
        self.observation_space = spaces.Box(low=-1, high=1, shape=StateDim, dtype=float32)
        ...
        self.latest_observation = zeros(StateDim, dtype=float32)
    ...
    def sendMsg(self, msg):
        msg_len = len(dumps(msg).encode("utf-8"))
        msg_len_data = pack('i', msg_len)
        self.Red_client.send(msg_len_data)
        self.Red_client.send(dumps(msg).encode("utf-8"))

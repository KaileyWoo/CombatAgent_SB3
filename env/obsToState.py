...
class ObsToState:
    ...
    def getStateRewardDone(self, first_info, second_info, third_info, time):
        ...
        self.TotalReward = self.TotalReward + self.CurTotalReward

        if terminate_prob > 0.7:
            self.isDone = 1
            print("----------------------------------------------目标达成！-------------------------------------------------")
            print("本局时间: ", self.CurTime, ", Cur_potentialR=", self.CurPotentialReward, ", Next_potentialR=", next_potential_reward,
                  ", TotalReward=", self.TotalReward)
            print(", 高度:", cur_self_track['Altitude'], ", distance=", self.CurDistance,
                  ", attackAngle=", self.CurAttackAngle*180/math.pi, ", escapeAngle=", self.CurEscapeAngle*180/math.pi)
        else:
            self.isDone = self.is_termination(cur_self_track)
    ...
    def getOneFrameState(self, self_track, detectedInfo):
        ...
        # 高度[1000, 10000]
        self_alt = self_track['Altitude'] / 10000.0
        dete_alt = detectedInfo['Altitude'] / 10000.0
        # 速度分量[0, 600]
        ...
        one_state = np.array([lon_diff, lat_diff, self_alt, dete_alt, self_v_n, self_v_e, self_v_d, v_diff,
                              self_heading, self_pitch, self_roll, dete_heading, dete_pitch, dete_roll], dtype=np.float32)
    ...
    def getReward(self, self_info, dete_info, curFlag=False):
        ...
        attackAngle = math.acos((selfPlaneSpeedVector[0] * distanceVector3D[0] + selfPlaneSpeedVector[1] * distanceVector3D[1] +
                                 selfPlaneSpeedVector[2] * distanceVector3D[2]) / distance)
        escapeAngle = math.acos((detePlaneSpeedVector[0] * distanceVector3D[0] + detePlaneSpeedVector[1] * distanceVector3D[1] +
        ...
        potential_reward = (1-(math.fabs(attackAngle)/math.pi+math.fabs(escapeAngle)/math.pi))*\
                           math.exp(-0.0003*math.fabs(distance-self.WEZ_Min))
    ...
    def is_termination(self, cur_self_info):
        #判断是否结束
        Done = 0
        if cur_self_info['Altitude'] > 10000 or cur_self_info['Altitude'] < 1000:
            Done = 2
            print("高度越界，结束  时间: ", self.CurTime, ", Cur_potentialR=", self.CurPotentialReward,
                  ", TotalReward=", self.TotalReward, ", distance=", self.CurDistance)
        elif self.CurDistance > self.Distance_Max:
            Done = 2
            print("超出最大距离范围，结束  时间: ", self.CurTime, ", Cur_potentialR=", self.CurPotentialReward, ", TotalReward=", self.TotalReward)
        elif self.CurTime > 299:
            Done = 3
            print("---超时---，", ", Cur_potentialR=", self.CurPotentialReward, "TotalReward=", self.TotalReward, ", distance=", self.CurDistance)
